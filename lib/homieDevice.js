...
var HomieDevice = module.exports = function(config) {
  ...
  t.device_id = t.config.device_id
...
proto.node = function(id, type, name) {
  var t = this;
  return t.nodes[id] = new HomieNode(t, id, type, name);
}
...
proto.onMessage = function(topic, msg) {
  ...
  if (parts[1] == t.device_id && parts[4] == 'set') {
    var nodeId = parts[2];
    ...
    if (node) {
      ...
      // See if the topic has a range
      if (!prop) {
        var parts = propName.split('_');
        if (parts.length > 1) {
          propName = parts.slice(0, parts.length - 1).join('_');
          prop = node.props[propName];
          if (prop) {
            range.isRange = true;
            range.index = +(parts.slice(-1));
          }
        }
      }
